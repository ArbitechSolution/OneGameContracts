...
contract LSSVMRouter {
    ...
    function getBUYvalues(PairSwapSpecific[] calldata swapList) public view returns(uint256 pairCost){
         // Do swaps
        uint256 numSwaps = swapList.length;
        for (uint256 i; i < numSwaps; ) {
            // Calculate the cost per swap first to send exact amount of ETH over, saves gas by avoiding the need to send back excess ETH
    ...
    function getSELLvalues(PairSwapSpecific[] calldata swapList) public view returns(uint256 pairCost){
         // Do swaps
        uint256 numSwaps = swapList.length;
        for (uint256 i; i < numSwaps; ) {
            // Calculate the cost per swap first to send exact amount of ETH over, saves gas by avoiding the need to send back excess ETH
            (, , , pairCost, ) = swapList[i].pair.getSellNFTQuote(
                swapList[i].nftIds.length
            );
                        return pairCost;
